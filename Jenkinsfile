    ...
    stages {
        ...
        stage('Test SSH') {
            steps {
                ...
                def MAIN_IP = step([$class: 'Sh', script: "aws ec2 describe-instances --region us-east-1 --filters \"Name=tag:Name,Values=main-instance-*\" --query 'Reservations[].Instances[].PublicIpAddress' --output text", returnStdout: true]).trim()
